...
/// How many jobs are fetched at a time.
///
/// One page is one round trip, so a job type with a lot of history
/// shows its newest jobs straight away instead of after the whole
/// limit has been walked.
const JOB_PAGE_SIZE: u32 = 25;
...
pub(super) async fn update_jobs(
    ...
    let event = match request(
        "jobs",
        admin.list_jobs_page(filters, order, JOB_PAGE_SIZE, page_token),
    )
    .await
    {
        ...
        Err(error) => AppEvent::Failed(Request::Jobs, error, Some(generation)),
    };
...
pub(super) async fn update_schedules(
    ...
    .await
    {
        ...
        Err(error) => AppEvent::Failed(Request::Schedules, error, Some(generation)),
    };
