...
fn cell_spans(text: &str, width: usize, active: bool, cursor: usize) -> Vec<Span<'static>> {
    ...
    let window = width.max(4);
    let start = (cursor + 1).saturating_sub(window);
...
fn row_spans(
    ...
    spans.push(
        Span::from(chars[start..cursor.min(end)].iter().collect::<String>()).style(value_style),
    );

    spans.push(
        Span::from(chars.get(cursor).copied().unwrap_or(' ').to_string())
            .style(value_style.add_modifier(Modifier::REVERSED)),
    );

    if cursor < end {
        spans
            .push(Span::from(chars[cursor + 1..end].iter().collect::<String>()).style(value_style));
    }
...
enum FieldKind {
    ...
    Bool,
    Choice(Vec<String>),
    /// A list of scalars, edited one value per row.
    List,
...
pub(crate) fn parse_cron(text: &str) -> Result<Timestamp, String> {
    ...
    let crontab =
        cronexpr::parse_crontab_with(text.trim(), options).map_err(|error| cron_error(&error))?;
...
impl Field {
    ...
    fn display(&self) -> String {
        match &self.kind {
            ...
            FieldKind::List => self.items.join(", "),
...
impl Form {
    ...
    fn active_text(&self) -> Option<&String> {
        ...
        match field.kind {
            FieldKind::List => field.items.get(self.item),
            FieldKind::Pairs => field
                .pairs
                .get(self.item)
                .map(|pair| if self.cell == 0 { &pair.0 } else { &pair.1 }),
            FieldKind::Text | FieldKind::Integer | FieldKind::Time | FieldKind::Cron => {
                Some(&field.text)
            }
            _ => None,
        }
    ...
    fn rows_in(&self, index: usize) -> usize {
        match self.fields.get(index) {
            Some(field) if matches!(field.kind, FieldKind::List) => field.items.len() + 1,
...
fn collect_fields(root: &Value, node: &Value, path: &[String], out: &mut Vec<Field>) {
    ...
    for (name, property) in properties {
        ...
        if kind_name.as_deref() == Some("object") && property.get("properties").is_some() {
            collect_fields(root, &property, &path, out);
            continue;
        }
...
fn field_kind(root: &Value, property: &Value, kind_name: Option<&str>) -> FieldKind {
    ...
    match kind_name {
        ...
        Some("integer" | "number") => FieldKind::Integer,
        Some("array") => FieldKind::List,
        _ => FieldKind::Text,
    }
}
...
fn apply_default(field: &mut Field, default: Option<&Value>) {
    ...
    match (&field.kind, default) {
        ...
        (FieldKind::List, Value::Array(items)) => {
            field.items = items
                .iter()
        ...
        Self: Sized,
    {
        ...
        for (index, field) in self.fields.iter().enumerate() {
            ...
            if matches!(field.kind, FieldKind::Cron) && !field.text.trim().is_empty() {
                let (text, style) = match parse_cron(&field.text) {
                    Ok(next) => (
                        format!(
                            "  → next {} ({})",
                            super::format_time(Some(next)),
                            super::format_age(Some(next))
                        ),
            ...
            if matches!(field.kind, FieldKind::Time) && !field.text.trim().is_empty() {
                let (text, style) = match parse_time(&field.text) {
                    Ok(timestamp) => (
                        format!(
                            "  → {} ({})",
        ...
        if let Some(error) = self.error.as_ref() {
            ...
            let hint = match field.kind {
                FieldKind::List => "Press `,` or ↓ for another value. Emptying a row removes it.",
                FieldKind::Pairs => {
