...
impl Widget for &mut Form {
        ...
        Self: Sized,
    {
        ...
        // Wide enough for the longest label in this form, so every
        // row's value lines up without wasting room on the others.
        ...
        for (index, field) in self.fields.iter().enumerate() {
            ...
            if matches!(field.kind, FieldKind::Pairs) {
                ...
                for row in 0..rows {
                    ...
                    spans.extend(cell_spans(&key, key_width, key_active, self.cursor));
                    // A space of its own, so the key never touches "="
                    // even when it fills the whole column.
                    ...
                    if selected || !value.trim().is_empty() {
                        ...
                        spans.extend(cell_spans(
                            &value,
                            budget.saturating_sub(key_width + 3),
