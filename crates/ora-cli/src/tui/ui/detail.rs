...
use ora::{
    ...
    proto::admin::v1::{Execution, Job, Schedule, ScheduleStatus},
};
use ratatui::{
...
fn job_overview(job: &Job) -> Vec<Line<'static>> {
    ...
    if let Some(def) = job.job.as_ref() {
        ...
        if let Some(timeout) = def.timeout_policy.as_ref().and_then(|p| p.timeout) {
            lines.push(field("Timeout", format_duration(Some(timeout))));
        }

        if let Some(retry) = def.retry_policy.as_ref() {
            lines.push(field(
                "Retries",
                format!(
                    "{} ({}, backoff {})",
...
fn schedule_overview(schedule: &Schedule) -> Vec<Line<'static>> {
    ...
    if let Some(def) = schedule.schedule.as_ref() {
        ...
        if let Some(template) = def.job_template.as_ref() {
            if let Some(timeout) = template.timeout_policy.as_ref().and_then(|p| p.timeout) {
                lines.push(field("Timeout", format_duration(Some(timeout))));
            }

            if let Some(retry) = template.retry_policy.as_ref() {
                lines.push(field(
                    "Retries",
                    format!(
                        "{} ({}, backoff {})",
